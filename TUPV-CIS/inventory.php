<?php
    ...
    $qty = $row["qty"];
        ...
        $sql2 = "SELECT SUM(givento)`givento` FROM med_inventory WHERE `status` = 'NOT EXPIRED' ";
...
<body>
    ...
    <div class="container">
         <h2> <span class="text-danger">I</span>NVENTORY </h2>
         <a type="button" href="inventoryHistory.php" class="btn btAN  col-auto mb-2 bt shadow-none fs-6 float-right">Full List</a>
         <h6 class="float-right col-4">DEDUCTED STOCK: <span class = "fs-5 text-danger"><?= $gib?></span> </h6>
        <h6 class="float-right col-4">TOTAL STOCK: <span class = "fs-5 text-danger"><?= $qty?></span> </h6>
         ...
         <table class = "table table-striped table-hover table-sm">
            ...
            // $sql = "SELECT * FROM med_inventory WHERE `status` LIKE 'NOT EXPIRED'";
            $sql = "SELECT `id`,`MedicineName`, `ExpDate`, SUM(qty) `qty`,SUM(givento) `givento`, `status` FROM med_inventory WHERE `status` = 'NOT EXPIRED' GROUP BY `MedicineName`";
            ...
            while($row = $result-> fetch_assoc()){
                ...
                echo'<tr>'
                    ...
                    .'<td data-title="Given Today">'. $row["givento"] .'</td>'
